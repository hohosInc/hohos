$(function () {
  ...
  //   if(to_user !=undefined && post!=undefined){
  //   $.ajax({
  //     url: '/feeds/post/',
  ...
  $("ul.stream, ul.stream_sidebar").on("click", ".comment", function () { 
    ...
    if ($(".comments", post).hasClass("tracking")) {
      ...
      $.ajax({
        ...
        cache: false,
        beforeSend: function () {
          $("ol", post).html("<li class='loadcomment'><i class='fa fa-spinner fa-spin fa-1x'></i>");
        },
        success: function (data) {
          $("ol", post).html(data);
...
$("ul.stream").on("click",".commentbtn",function(){   // on click listener on cmntbtn in mid feeds not in side feeds
  ...
  $("#compose-form input[name='post']").keyup(function () {
    $(this).count(255);
  });
